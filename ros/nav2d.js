...
NAV2D.Navigator = function (options) {
  ...
  WP_array.subscribe(function (data) {
    ...
    //   route[i - 1] = new ROS2D.NavigationRoute({
    //     size: 3,
    //     strokeSize: 1,
    //     strokeColor: createjs.Graphics.getRGB(0, 0, 255,),
    //   });
    //   route[i - 1].x = data.poses[goalMarkerCounter].position.x;
    //   route[i - 1].y = -data.poses[goalMarkerCounter].position.y;goalMarker[goalMarkerCounter].scaleX = 1.0 / stage.scaleX;
    //   route[i - 1].scaleX = 1.0 / stage.scaleX;
    //   route[i - 1].scaleY = 1.0 / stage.scaleY;
    //   that.rootObject.addChild(route[i - 1]);
    // }
